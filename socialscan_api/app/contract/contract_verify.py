...
VERIFY_HOST = app_config.contract_service
...
def send_sync_verification_request(payload, file_list):
    ...
    return requests.post(CONTRACT_VERIFY_URL, data=payload, files=files)
...
def send_async_verification_request(payload):
    ...
    response = requests.post(COMMON_CONTRACT_VERIFY_URL, data=payload, files=files)
    return response
...
def get_creation_or_deployed_code(contract: Contracts):
    # trace = db.session.query(Traces).filter(
    #     Traces.to_address == contract.address and Traces.trace_type.in_(['create', 'create2'])).first()
    # return trace.input, trace.output
...
def get_contract_names(address_list: list[str]) -> dict[str:str]:
    ...
    response = requests.post(CONTRACT_NAME_URL, json=request_json)
