...
FINALIZE_BRIDGE_ERC721 = cast(ABIFunction, json.loads("""
    {
        "type": "function",
...
def decode_function(input_data: bytes) -> BridgeRemoteFunctionCallInfo:
    ...
    function_info = decode_transaction_data(
        FINALIZE_BRIDGE_ERC721, input_data.hex()
    )
    ...
    return BridgeRemoteFunctionCallInfo(
        ...
        remove_function_call_type=BedRockFunctionCallType.DEPOSIT_ERC721.value
    )


FINALIZE_BRIDGE_ERC721_DECODER = RemoteFunctionCallDecoder(
    function_abi=FINALIZE_BRIDGE_ERC721,
