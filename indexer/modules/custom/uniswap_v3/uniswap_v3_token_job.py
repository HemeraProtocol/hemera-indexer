...
class ExportUniSwapV3TokensJob(FilterTransactionDataJob):
    ...
    def _collect(self, **kwargs):
        ...
        # collect new token_id

        logs = self._data_buff[Log.type()]
        new_collect_token_id_info = []
        new_collect_token_id_set = set()
        ...
        for log in logs:
            ...
            elif topic0 not in self._liquidity_topic0_dict:
                continue
            else:
                topic_index = self._liquidity_topic0_dict[topic0]
                if topic_index == 1:
            ...
            if token_id not in self._exist_token_ids and token_id not in need_collect_token_id_set:
                new_collect_token_id_info.append(data)
                new_collect_token_id_set.add(token_id)
            if key not in new_collect_token_id_set:
