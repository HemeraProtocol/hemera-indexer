...
class ExportPancakeTokensJob(FilterTransactionDataJob):
    dependency_types = [Log, ERC721TokenTransfer, Block]
    output_types = [PancakeToken, PancakeTokenDetail, PancakeTokenCurrentStatus]
    able_to_reorg = True
    ...
    def __init__(self, **kwargs):
        ...
        config = kwargs.get("config")["pancake_job"]
    ...
    def _collect(self, **kwargs):
        ...
        for data in new_nft_info:
            self._collect_item(PancakeToken.type(), data)
        ...
        for data in token_result:
            self._collect_item(PancakeTokenDetail.type(), data)
        for data in current_statuses:
            self._collect_item(PancakeTokenCurrentStatus.type(), data)
        for token_id, block_number in burn_token_ids.items():
            self._collect_item(
                PancakeTokenDetail.type(),
                PancakeTokenDetail(
            ...
            self._collect_item(
                PancakeTokenCurrentStatus.type(),
                PancakeTokenCurrentStatus(
                    position_token_address=self._position_token_address,
                    pool_address=self._exist_token_ids.get(token_id, ""),
                    token_id=token_id,
        ...
        self._data_buff[PancakeTokenCurrentStatus.type()] = distinct_collections_by_group(
            self._data_buff[PancakeTokenCurrentStatus.type()], ["position_token_address", "token_id"], "block_number"
        )

        self._block_infos = {}

    def _process(self, **kwargs):
        self._data_buff[PancakeToken.type()].sort(key=lambda x: x.block_number)
        self._data_buff[PancakeTokenDetail.type()].sort(key=lambda x: x.block_number)
        self._data_buff[PancakeTokenCurrentStatus.type()].sort(key=lambda x: x.block_number)
