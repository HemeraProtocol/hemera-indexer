...
from indexer.modules.custom.uniswap_v3.constants import AGNI_ABI  # should be the same
from indexer.modules.custom.uniswap_v3.domain.feature_uniswap_v3 import (
    FusionxPool,
    FusionxPoolCurrentPrice,
    FusionxPoolPrice,
)
...
class ExportFusionxPoolJob(FilterTransactionDataJob):
    dependency_types = [Log]
    output_types = [FusionxPool, FusionxPoolPrice, FusionxPoolCurrentPrice]
    able_to_reorg = True
    ...
    def __init__(self, **kwargs):
        ...
        self._load_config("fusionx_config.ini", self._chain_id)
    ...
    def get_filter(self):
        ...
        pool_address_list = ["0x6b6981dff307536f2781231d9c680847359164e1",
                             "0xc715b2759beecfb83faab47161c871044c23535f",
                             "0xe7eb4989957f53fe4bd3f9854f4be5dd38a746f3",
                             "0x8a6a1ed01989ff1c5ac6361c34cad9d7d0015ab4",
                             "0xefe8bfd40352b9c14b64c06f5e699b3d46ca4ffc",
                             "0x11456b1622f2bdbbe54380b1f17d9ef71ad4e555",
                             "0xc522a2108fb700952d42acec956d332f1856d7f9",
                             "0x283fdeeac018e403a9274fbdec3c570dd3157e68",
                             "0x6f4c4caed9e97d5a9146944af740a706cffa07d9"]

        return TransactionFilterByLogs(
            [
                TopicSpecification(topics=liquidity_topic_keys, addresses=pool_address_list),
            ]
        )
    ...
    def _collect(self, **kwargs):
        ...
        for pools in format_pool_item(need_add_in_exists_pools):
            self._collect_item(FusionxPool.type(), pools)
    ...
    def _collect_batch(self, logs_dict):
        ...
        current_price = None
        for price in prices:
            self._collect_item(FusionxPoolPrice.type(), price)
            if current_price is None or price.block_number > current_price.block_number:
                current_price = FusionxPoolCurrentPrice(
        ...
        if current_price:
            self._collect_item(FusionxPoolCurrentPrice.type(), current_price)

    def _process(self, **kwargs):
        self._data_buff[FusionxPool.type()].sort(key=lambda x: x.block_number)
        self._data_buff[FusionxPoolPrice.type()].sort(key=lambda x: x.block_number)
        self._data_buff[FusionxPoolCurrentPrice.type()].sort(key=lambda x: x.block_number)


def format_pool_item(new_pools):
    result = []
    for pool_address, pool in new_pools.items():
        result.append(dict_to_dataclass(pool, FusionxPool))
    return result
