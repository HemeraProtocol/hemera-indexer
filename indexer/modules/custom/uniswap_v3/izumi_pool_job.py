...
class ExportIzumiPoolJob(FilterTransactionDataJob):
    ...
    def get_pools(self, logs):
            ...
            if log_address not in self._exist_pools:
                ...
                if log_address == self._factory_address and log.topic0 == POOL_CREATED_EVENT.get_signature():
                    ...
                    uniswap_v3_pool = IzumiPool(
                        block_timestamp=log.block_timestamp, factory_address=self._factory_address, pool_id=None, **new_pool_dict
                    )
        ...
        # Add poolid to poo list
        new_pool_info_list = []
        for pool in new_pool_list:
            new_pool_info_list.append({
                "param_to": self._position_token_address,
                "param_data": GET_POOL_ID_FUNCTION.encode_function_call_data([pool.pool_address]),
                "param_number": pool.block_number,
                "request_id": str(pool.block_number) + "-" + pool.pool_address + "-" + GET_POOL_ID_FUNCTION.get_signature(),
                "block_timestamp": pool.block_timestamp,
                "pool_address": pool.pool_address,
            })
        print(new_pool_info_list)
        
        if len(new_pool_info_list) > 0:
            response = self._batch_web3_provider.make_request(
                params=orjson.dumps(list(generate_eth_call_json_rpc(new_pool_info_list)))
            )

            pool_id_mapping = {}
            for data in list(zip_rpc_response(new_pool_info_list, response)):
                result = rpc_response_to_result(data[1])
                pool_info = data[0]
                try:
                    decoded_data = decode_data(GET_POOL_ID_FUNCTION.get_outputs_type() ,hex_str_to_bytes(result))
                    pool_id_mapping[pool_info["pool_address"]] = decoded_data[0]

                except Exception as e:
                    logger.error(
                        f"Decoding pool id failed. "
                        f"pool info: {pool_info}. "
                        f"rpc response: {result}. "
                        f"exception: {e}"
                    )
            print(pool_id_mapping)
            for pool in new_pool_list:
...
def collect_swap_new_pools(
    ...
    factory_infos = common_utils.simple_get_rpc_requests(
        web3, make_requests, swap_pools, is_batch, abi_list, FACTORY_FUNCTION.get_name(), "address", batch_size, max_worker
    )
    uniswap_pools = []
    need_add = {}
    ...
    token0_infos = common_utils.simple_get_rpc_requests(
        web3, make_requests, uniswap_pools, is_batch, abi_list, "tokenX", "address", batch_size, max_worker
    )
    token1_infos = common_utils.simple_get_rpc_requests(
        web3, make_requests, token0_infos, is_batch, abi_list, "tokenY", "address", batch_size, max_worker
    )
    tick_infos = common_utils.simple_get_rpc_requests(
        web3, make_requests, token1_infos, is_batch, abi_list, "pointDelta", "address", batch_size, max_worker
    )
    ...
    for data in tick_infos:
        ...
        if "tokenX" in data and "tokenY" in data and "pointDelta" in data:
            new_pool = {
                "position_token_address": position_token_address,
                "token0_address": data["tokenX"],
                "token1_address": data["tokenY"],
                # "fee": data["fee"],
                "tick_spacing": data["pointDelta"],
                "pool_address": pool_address,
                "block_number": data["block_number"],
                "block_timestamp": data["block_timestamp"],
            }
...
def slot0_rpc_requests(web3, make_requests, requests, is_batch, abi_list, batch_size, max_worker):
    if len(requests) == 0:
        return []
    fn_name = SLOT0_FUNCTION.get_name()
