...
class ExportUniSwapV3TokensJob(FilterTransactionDataJob):
    ...
    def get_filter(self):
        return TransactionFilterByLogs([
            TopicSpecification(addresses=[self._factory_address], topics=list(self._liquidity_topic0_dict.keys())),
    ...
    def _collect(self, **kwargs):
        ...
        mint_token_ids, burn_token_ids, all_token_dict = extract_changed_tokens(
            ...
            self._nft_address)
        token_id_liquidity_records = extract_liquidity_logs(self._liquidity_topic0_dict,
                                                            self._data_buff[Log.type()],
                                                            self._nft_address)

        need_collect_value_tokens, want_pool_tokens = gather_collect_infos(all_token_dict,
                                                                           token_id_liquidity_records,
                                                                           burn_token_ids,
                                                                           self._exist_token_ids)
        # call owners
        owner_dict = get_owner_dict(self._web3,
                                    self._batch_web3_provider.make_request,
                                    need_collect_value_tokens,
                                    self._nft_address,
                                    self._is_batch, self._abi_list)

        # call positions
        token_infos = positions_rpc_requests(self._web3,
                                             self._batch_web3_provider.make_request,
                                             need_collect_value_tokens,
                                             self._nft_address,
                                             self._is_batch,
        ...
        update_exist_tokens, new_nft_info = get_new_nfts(token_infos, want_pool_tokens, self._nft_address, self._web3,
                                                         self._batch_web3_provider.make_request,
                                                         need_collect_value_tokens,
...
def get_exist_token_ids(db_service, nft_address):
    ...
    try:
        result = session.query(UniswapV3Tokens.token_id, UniswapV3Tokens.pool_address) \
            .filter(
            UniswapV3Tokens.pool_address != None,
            UniswapV3Tokens.nft_address == bytes.fromhex(nft_address[2:])).all()
        history_token = {}
...
def build_token_id_method_data(web3, token_ids, nft_address, fn, abi_list):
    ...
    for idx, token in enumerate(token_ids):
        ...
        try:
            ...
            logger.error(f"Encoding token id {token['token_id']} for function {fn} failed. "
...
def positions_rpc_requests(web3, make_requests, requests, nft_address, is_batch, abi_list):
    if len(requests) == 0:
        return []
    fn_name = 'positions'
    function_abi = next((abi for abi in abi_list if abi['name'] == fn_name and abi['type'] == 'function'),
                        None)
    ...
    token_name_rpc = list(
        generate_eth_call_json_rpc(parameters))
    if is_batch:
    ...
    for data in list(zip_rpc_response(parameters, response)):
        ...
        try:
            ...
            logger.error(f"Decoding positions failed. "
                         f"token: {token}. "
                         f"fn: {fn_name}. "
...
def owner_rpc_requests(web3, make_requests, requests, nft_address, is_batch, abi_list):
    ...
    for data in list(zip_rpc_response(parameters, response)):
        ...
        try:
            ...
            logger.error(f"Decoding ownerOf failed. "
                         f"token: {token}. "
                         f"fn: {fn_name}. "
                         f"rpc response: {result}. "
                         f"exception: {e}")
...
def build_get_pool_method_data(web3, requests, factory_address, fn, abi_list):
    ...
    for idx, token in enumerate(requests):
        ...
        try:
            # Encode the ABI for the specific token_id
            data = contract.encodeABI(fn_name=fn, args=[Web3.to_checksum_address(token['token0']),
                                                        Web3.to_checksum_address(token['token1']), token['fee']])
            token['param_data'] = data
        except Exception as e:
            logger.error(
...
def get_pool_rpc_requests(web3, make_requests, requests, factory_address, is_batch, abi_list):
    ...
    for data in list(zip_rpc_response(parameters, response)):
        ...
        try:
            ...
            logger.error(f"Decoding getPool failed. "
                         f"token: {token}. "
                         f"fn: {fn_name}. "
                         f"rpc response: {result}. "
                         f"exception: {e}")
    ...
    for idx, token in enumerate(requests):
        ...
        try:
            ...
            token['param_data'] = (web3.eth
                                   .contract(address=Web3.to_checksum_address(token['pool_address']),
                                             abi=abi_list)
