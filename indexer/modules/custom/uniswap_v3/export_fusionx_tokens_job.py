...
class ExportFusionxTokensJob(FilterTransactionDataJob):
    dependency_types = [Log, ERC721TokenTransfer, Block]
    output_types = [FusionxToken, FusionxTokenDetail, FusionxTokenCurrentStatus]
    able_to_reorg = True
    ...
    def _collect(self, **kwargs):
        ...
        for data in new_nft_info:
            self._collect_item(FusionxToken.type(), data)
        ...
        for data in token_result:
            self._collect_item(FusionxTokenDetail.type(), data)
        for data in current_statuses:
            self._collect_item(FusionxTokenCurrentStatus.type(), data)
        for token_id, block_number in burn_token_ids.items():
            self._collect_item(FusionxTokenDetail.type(), FusionxTokenDetail(
            ...
            self._collect_item(FusionxTokenCurrentStatus.type(), FusionxTokenCurrentStatus(
                nft_address=self._nft_address,
                pool_address=self._exist_token_ids.get(token_id, ""),
                token_id=token_id,
                wallet_address=constants.ZERO_ADDRESS,
        ...
        self._data_buff[FusionxTokenCurrentStatus.type()] = distinct_collections_by_group(
            self._data_buff[FusionxTokenCurrentStatus.type()], ['nft_address', 'token_id'], 'block_number')

        self._block_infos = {}

    def _process(self, **kwargs):
        self._data_buff[FusionxToken.type()].sort(key=lambda x: x.block_number)
        self._data_buff[FusionxTokenDetail.type()].sort(key=lambda x: x.block_number)
        self._data_buff[FusionxTokenCurrentStatus.type()].sort(key=lambda x: x.block_number)
