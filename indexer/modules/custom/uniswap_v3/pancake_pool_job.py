...
class ExportPancakePoolJob(FilterTransactionDataJob):
    dependency_types = [Log]
    output_types = [PancakePool, PancakePoolCurrentPrice, PancakePoolPrice, PancakeSwapEvent]
    able_to_reorg = True
    ...
    def __init__(self, **kwargs):
        ...
        config = kwargs["config"]["pancake_job"]
    ...
    def get_pools(self):
        ...
        for pool_address, pool_dict in pools_get_from_swap_event.items():
            if pool_address not in self._exist_pools:
                ...
                uniswap_v3_pool = PancakePool(factory_address=self._factory_address, fee=0, **pool_dict)
    ...
    def _collect_batch(self, logs_dict):
        ...
        current_price = None
        for price in prices:
            self._collect_item(PancakePoolPrice.type(), price)
            if current_price is None or price.block_number > current_price.block_number:
                current_price = PancakePoolCurrentPrice(
        ...
        if current_price:
            self._collect_item(PancakePoolCurrentPrice.type(), current_price)

    def _process(self, **kwargs):
        self._data_buff[PancakePool.type()].sort(key=lambda x: x.block_number)
        self._data_buff[PancakePoolPrice.type()].sort(key=lambda x: x.block_number)
        self._data_buff[PancakePoolCurrentPrice.type()].sort(key=lambda x: x.block_number)
