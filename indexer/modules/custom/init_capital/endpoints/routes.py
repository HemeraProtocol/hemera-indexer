...
from common.utils.format_utils import hex_str_to_bytes, bytes_to_hex_str, as_dict
from indexer.modules.custom.init_capital.models.init_capital_models import InitCapitalPositionCurrent, InitCapitalRecords, InitCapitalPoolCurrent
...
def get_address_positions(address: Union[str, bytes]):
    ...
    positions = db.session.query(InitCapitalPositionCurrent).filter(
        InitCapitalPositionCurrent.viewer_address == address_bytes
    ).all()
            ...
            for collateral in position.collaterals:
                if collateral["amount"] != 0:
                    if collateral["token_address"] not in collaterals_token_map:
                        collaterals_token_map[collateral["token_address"]] = {
                            "amount": 0
                        }
    ...
    token_infos = db.session.query(InitCapitalPoolCurrent).filter(
        InitCapitalPoolCurrent.token_address.in_(token_bytes_list)
    ).all()
    token_info_map = {}
    ...
    for token in collaterals_token_map:
        ...
        collateral["true_amount"] = str(collateral["amount"] * (token_info_map[token].total_asset + VIRTUAL_ASSETS) / (token_info_map[token].total_supply + VIRTUAL_SHARES))
    
    for token in borrow_token_map:
        borrow = borrow_token_map[token]
        borrow["true_amount"] = str(borrow["share"] * token_info_map[token].total_debt / token_info_map[token].total_debt_share)
