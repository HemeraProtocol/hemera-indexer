...
from indexer.modules.custom.all_features_value_record import AllFeatureValueRecordUniswapV2TotalSupply
...
"""
`TokenBalance` is required.
need the ratio of the pool's `balanceOf` to `totalSupply`
"""


class ExportUniSwapV2InfoJob(FilterTransactionDataJob):
    dependency_types = [Log, TokenBalance]
    output_types = [AllFeatureValueRecordUniswapV2TotalSupply, UniswapV2Pool]
    ...
    def _process(self):
        ...
        for record in format_value_records(self._exist_pools, self._collected_total_supply, FEATURE_ID):
            self._collect_item(AllFeatureValueRecordUniswapV2TotalSupply.type(), record)

        self._data_buff[UniswapV2Pool.type()].sort(key=lambda x: x.called_block_number)
        self._data_buff[AllFeatureValueRecordUniswapV2TotalSupply.type()].sort(key=lambda x: x.block_number)
...
def format_value_records(exist_pools, pool_total_supply, feature_id):
    ...
    for data in pool_total_supply:
        ...
        result.append(
            AllFeatureValueRecordUniswapV2TotalSupply(
