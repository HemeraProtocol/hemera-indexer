...
from indexer.utils.utils import rpc_response_to_result, ZERO_ADDRESS
...
def abi_selector_encode_and_decode_type(token_id_info: TokenIdInfo):
    if token_id_info.token_type == TokenType.ERC721.value:
        if token_id_info.is_get_token_uri:
            return encode_abi(ERC721_TOKEN_URI_ABI_FUNCTION, [token_id_info.token_id], erc721_uri_sig_prefix)
        else:
            return encode_abi(ERC721_OWNER_OF_ABI_FUNCTION, [token_id_info.token_id], erc721_owner_of_sig_prefix)
    elif token_id_info.token_type == TokenType.ERC1155.value:
        if token_id_info.is_get_token_uri:
            return encode_abi(ERC1155_TOKEN_URI_ABI_FUNCTION, [token_id_info.token_id], erc1155_token_uri_sig_prefix)
        else:
            return encode_abi(ERC1155_TOTAL_SUPPLY_ABI_FUNCTION, [token_id_info.token_id],
                              erc1155_token_supply_sig_prefix)


def token_ids_info_rpc_requests(make_requests, token_info_items, is_batch):
    return_data = []
    eth_calls = list(generate_eth_call_json_rpc([{
        ...
        try:
            if token_info.token_type == "ERC721":
                if token_info.is_get_token_uri:
                    return_data.append(ERC721TokenIdDetail(
                    ...
                    return_data.append(UpdateERC721TokenIdDetail(
                        token_address=token_info.address,
                        token_id=token_info.token_id,
                    ...
                    return_data.append(ERC721TokenIdChange(
                        token_address=token_info.address,
                        token_id=token_info.token_id,
                        token_owner=abi.decode(['address'], bytes.fromhex(value))[0],
                        block_number=token_info.block_number,
                ...
                if token_info.is_get_token_uri:
                    return_data.append(ERC1155TokenIdDetail(
                        token_address=token_info.address,
                        token_id=token_info.token_id,
                        token_uri=abi.decode(['string'], bytes.fromhex(value))[0].replace('\u0000', ''),
                        block_number=token_info.block_number,
                    ...
                    return_data.append(UpdateERC1155TokenIdDetail(
                        token_address=token_info.address,
                        token_id=token_info.token_id,
