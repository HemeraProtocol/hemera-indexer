...
-- use af_staked_transferred_balance_hist for fbtc token, now just for arbitrum
with sbtc_config as (select distinct contract_address, token_address
                     from feature_staked_fbtc_config),

     period_af_staked_transferred_balance_hist as (select *
                                                   from (select *,
                                                                row_number()
                                                                over (partition by contract_address, wallet_address, token_address order by block_timestamp desc) rn
                                                         from af_staked_transferred_balance_hist
                                                         where to_timestamp(block_timestamp) < '{end_date}') t
                                                   where rn = 1),

     fbtc_staked_records as (select wallet_address,
                                    contract_address,
                                    token_address,
                                    protocol_id,
                                    block_cumulative_value
                             from period_af_staked_transferred_balance_hist
                             where token_address = '\xC96DE26018A54D51C097160568752C4E3BD6C364'),
...
-- 同样逻辑应用到 protocol 层级
     ...
     a_balance_table as (select d1.wallet_address,
                                d1.contract_address,
                                d1.protocol_id,
                                d1.block_cumulative_value / pow(10, d2.decimals) as a_balance,
                                d2.address                                       as fbtc_token_address
                         from fbtc_staked_records d1
