...
class PeriodFeatureDefiWalletFbtcAggregates:
    ...
    def get_pool_token_data(self, orm_list):
        ...
        for period_date_wallet, protocol_contract_group in wallet_protocol_contract_group.items():
            ...
            for protocol_id, contract_group in protocol_contract_group.items():
                protocol_json = {"pool_data": [], "protocol_id": protocol_id}
            ...
            results[(period_date, wallet_address)] = {
                "contract_json": wallet_address_json,
    ...
    def get_latest_price(self, symbol_list):
        ...
        subquery = (
            session.query(
                ...
                TokenPrice.price,
                func.row_number()
                .over(partition_by=TokenPrice.symbol, order_by=desc(TokenPrice.timestamp))
                .label("row_number"),
            )
            .filter(TokenPrice.timestamp <= price_date_limit)
            .filter(TokenPrice.symbol.in_(symbol_list))
            .subquery()
        )

        latest_per_address = session.query(subquery).filter(subquery.c.row_number == 1).all()
    ...
    def get_filter_fbtc_orm(self, orm_class):
        ...
        orm_list = (
            ...
            .filter(orm_class.period_date == self.start_date)
            .filter(or_(orm_class.token0_symbol == "FBTC", orm_class.token1_symbol == "FBTC"))
            .filter(and_(or_(orm_class.token0_balance > 0, orm_class.token1_balance > 0)))
            .all()
        )
        session.close()
        return orm_list
    ...
    def get_staked_json(self):
        ...
        fbtc_detail_list = (
            session.query(PeriodFeatureHoldingBalanceStakedFbtcDetail)
            .filter(PeriodFeatureHoldingBalanceStakedFbtcDetail.period_date == self.start_date)
            .all()
        )
        ...
        for k, entity_list in grouped_data.items():
            ...
            for entity in entity_list:
                ...
                protocol_balance += balance
                protocol_usd += token_usd
            h = {"pool_data": token_json, "protocol_id": protocol_id}
            protocol_json.append(h)
            results[(period_date, wallet_address)] = {
                "contract_json": protocol_json,
                "balance": protocol_balance,
                "usd": protocol_usd,
            }
        session.close()
        return results
    ...
    def get_period_address_token_balances(self):
        ...
        period_addresses = (
            session.query(PeriodAddressTokenBalances).filter(PeriodAddressTokenBalances.token_address == address).all()
        )
    ...
    def get_lendle_json(self):
        session = self.db_service.Session()
        orm_list = (
            session.query(PeriodFeatureHoldingBalanceLendle)
            .filter(PeriodFeatureHoldingBalanceLendle.period_date == self.start_date)
            .all()
        )
    ...
    def run(self):
        ...
        address_token_balances = self.timed_call(
            self.get_period_address_token_balances, "get_period_address_token_balances"
        )
        ...
        protocol_wallet_keys_list = list(
            {
                key
                for d in [uniswapv3, staked, address_token_balances, merchantmoe_json, dodo_json, get_lendle_json]
                for key in d.keys()
            }
        )
