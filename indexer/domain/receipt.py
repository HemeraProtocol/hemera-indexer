...
class Receipt(Domain):
    ...
    def from_rpc(receipt_dict: dict, block_timestamp=None, block_hash=None, block_number=None):
        ...
        return Receipt(
            ...
            contract_address=to_normalized_address(receipt_dict.get('contractAddress'))
            if receipt_dict.get('contractAddress') else None,
            status=to_int(hexstr=receipt_dict['status']),
            logs=logs,
            root=receipt_dict.get('root'),
            cumulative_gas_used=to_int(hexstr=receipt_dict.get('cumulativeGasUsed'))
            if receipt_dict.get('cumulativeGasUsed') else None,
            gas_used=to_int(hexstr=receipt_dict.get('gasUsed')) if receipt_dict.get('gasUsed') else None,
            effective_gas_price=to_int(hexstr=receipt_dict.get('effectiveGasPrice'))
            if receipt_dict.get('effectiveGasPrice') else None,
            ...
            blob_gas_price=to_int(hexstr=receipt_dict.get('blobGasPrice')) if receipt_dict.get('blobGasPrice') else None
        )
