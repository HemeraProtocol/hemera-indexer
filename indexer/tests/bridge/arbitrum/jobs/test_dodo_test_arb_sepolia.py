...
from indexer.modules.bridge.domain.arbitrum import ArbitrumL1ToL2TransactionOnL1, ArbitrumL1ToL2TransactionOnL2, \
    ArbitrumL2ToL1TransactionOnL2, ArbitrumStateBatchConfirmed, ArbitrumStateBatchCreated, ArbitrumTransactionBatch, \
    ArbitrumL2ToL1TransactionOnL1
from indexer.tests import ARBITRUM_TESTNET_PUBLIC_NODE_RPC_URL, DODO_TESTNET_PUBLIC_NODE_RPC_URL, \
    ETHEREUM_PUBLIC_NODE_RPC_URL, ARBITRUM_PUBLIC_NODE_RPC_URL
...
def test_l1_to_l2_deposit_erc20():
    ...
    confirm = job_scheduler.get_data_buff().get(ArbitrumL2ToL1TransactionOnL2.type())
    confirm = [asdict(co) for co in confirm]
    assert confirm is not None
...
def test_l2_to_l1_withdraw():
    ...
    assert confirm[0]['value'] == 100000000000000000
